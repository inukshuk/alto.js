import { tags } from './dom.js'
import { AltoElement, Page, String, TextBlock } from './element.js'


export class Document extends AltoElement {
  scale = { x: 1, y: 1 }

  order = [
    'TopMargin',
    'PrintSpace',
    'LeftMargin',
    'RightMargin',
    'BottomMargin'
  ]

  get unit() {
    return this.get(
      'alto > Description > MeasurementUnit'
    ).textContent
  }

  get page() {
    return Page.for(this.get('alto > Layout > Page'))
  }

  *blocks(order = this.order) {
    let page = this.page
    if (page == null) return

    for (let name of order)
      for (let section of page.node.children) {
        if (section.localName !== name) continue

        let blocks = tags(section, 'TextBlock')

        for (let i = 0; i < blocks.length; i++)
          yield TextBlock.for(blocks[i])
      }
  }

  *lines(order = this.order) {
    for (let block of this.blocks(order))
      for (let line of block)
        yield line
  }

  *strings(order = this.order) {
    for (let block of this.blocks(order))
      for (let line of block)
        for (let string of line)
          yield string
  }

  // NB: index based on document order within each section!
  getStringAt(idx, order = this.order) {
    let page = this.page

    if (page == null || !(idx >= 0))
      return null

    for (let name of order)
      for (let section of page.node.children) {
        if (section.localName !== name) continue

        let strings = tags(section, 'String')

        if (idx < strings.length)
          return String.for(strings[idx])

        idx -= strings.length
      }

    return null
  }

  setScale(width, height) {
    let { WIDTH, HEIGHT } = this.page
    this.scale = {
      x: ((width ?? WIDTH) / WIDTH) || 1,
      y: ((height ?? HEIGHT) / HEIGHT) || 1
    }
  }

  select(rect = false, order = this.order) {
    let selection = new Map

    for (let string of this.strings(order)) {
      if (string.intersects(rect))
        selection.set(string, true)
    }

    return selection
  }

  range(head, tail, order = this.order) {
    let selection = new Map
    let isSelected = false
    let isAtomic = (tail == null || head === tail)

    for (let string of this.strings(order)) {
      let flip = (string === head || string === tail)

      if (isSelected) {
        selection.set(string, true)
        if (flip) break

      } else {
        if (flip) {
          selection.set(string, true)

          if (isAtomic) break
          else
            isSelected = true
        }
      }
    }

    return selection
  }

  toPlainText(selection = true, order = this.order) {
    if (selection == null || selection === false)
      return ''

    let text = []

    for (let block of this.blocks(order)) {
      let lines = []
      for (let line of block.lines()) {
        let strings = []
        for (let string of line.strings()) {
          if (selection === true || selection.get(string))
            strings.push(string.CONTENT)
        }
        if (strings.length)
          lines.push(strings)
      }
      if (lines.length)
        text.push(lines)
    }

    return text.map(block =>
      block.map(line => line.join(' ')).join('\n')
    ).join('\n')
  }
}
